...
const isRoomAvailable = (roomSchedules, roomId, day, startHour, duration) => {
  ...
  return !roomSchedules[roomId][day].some(
    (time) =>
...
const canScheduleProfessor = async (
  profSchedule,
  startHour,
  duration,
  settings,
  ...
  const intervals = [
    ...profSchedule.dailyTimes,
    { start: startHour, end: startHour + duration },
  ...
  try {
    ...
    const department = await Department.findByPk(DepartmentId, {
      include: [
        {
          model: Assignation,
          include: [Course, { model: Professor, attributes: ["id", "Name"] }],
        },
    ...
    for (const sch of existingSchedules) {
      ...
      professorSchedule[assign.Professor.id][day].dailyTimes.push({
        start: startH,
        end: endH,
      });
    ...
    const fullReport = allSchedules.map((sch) => ({
      ...
      Sections: sch.ProgYrSecs.map(
        (sec) => `ProgId=${sec.ProgramId}, Year=${sec.Year}, Sec=${sec.Section}`
      ),
      Room: sch.Room.Code,
      Day: sch.Day,
  ...
  try {
    ...
    if (courseParam.Type === "Core") {
      ...
      if (courseWithPrograms) {
        const allowed = courseWithPrograms.CourseProgs.map((p) => p.id).filter(
          (id) => validProgramIds.includes(id)
        );
        validProgYrSecs = await ProgYrSec.findAll({
          where: {
            Year: courseParam.Year,
    ...
    if (!validProgYrSecs.length) {
      ...
      for (let d = 1; d <= 6; d++) {
        progYrSecSchedules[placeholderSection.id][d] = {
          hours: 0,
          dailyTimes: [],
        };
      }
    ...
    if (
      priorities?.sections &&
      !validProgYrSecs[0].id.toString().includes("placeholder")
    ) {
    ...
    roomsToTry = roomsToTry.filter(
      (r) => r.RoomTypeId === assignation.RoomTypeId
    );
    ...
    for (const group of Object.values(sectionGroups)) {
      ...
      const daysWith = allDays.filter(
        (d) => progYrSecSchedules[firstSec.id][d].hours > 0
      );
      const daysWithout = allDays.filter(
        (d) => progYrSecSchedules[firstSec.id][d].hours === 0
      );
      ...
      for (const day of days) {
        ...
        for (const room of roomsToTry) {
          ...
          while (
            ...
            scheduledHours < settings.StudentMaxHours
          ) {
            const sectionIds = group.map((s) => s.id);
            const isPlaceholder = sectionIds.some(
              (id) => typeof id === "string" && id.includes("placeholder")
            );

            if (
              await isSchedulePossible(
                roomSchedules,
                professorSchedule,
                progYrSecSchedules,
                room.id,
...
const toggleLock = async (req, res, next) => {
  try {
    ...
    if (!schedule) {
      return res
        .status(404)
        .json({ successful: false, message: "Schedule not found." });
    }
...
const toggleLockAllSchedules = async (req, res) => {
  try {
    ...
    return res.json({
      successful: true,
      message: `Successfully ${isLocked ? "locked" : "unlocked"} ${
        scheduleIds.length
      } schedules`,
    });
  } catch (error) {
...
const addSchedule = async (req, res, next) => {
  try {
    ...
    for (const sched of schedule) {
      const { Day, Start_time, End_time, RoomId, AssignationId, Sections } =
        sched;
      ...
      if (sectionsData.length !== Sections.length) {
        return res.status(404).json({
          successful: false,
          message:
            "One or more sections not found. Please provide valid section IDs.",
        });
      }
      ...
      for (const sectionId of Sections) {
        ...
        sectionSchedules.forEach((s) => {
          ...
          sectionScheduleForDay.dailyTimes.push({
            start: sectionSchedStart,
            end: sectionSchedEnd,
          });
        });
        if (
          !canScheduleStudents(
            sectionScheduleForDay,
            newStartHour,
            currentScheduleDuration,
            settings
          )
      ...
      const createdScheduleWithSections = await Schedule.findByPk(
        newSchedule.id,
        {
          include: [ProgYrSec],
        }
...
const updateSchedule = async (req, res, next) => {
  try {
    ...
    if (
      !canScheduleProfessor(
        profScheduleForDay,
        newStartHour,
        updatedScheduleDuration,
        settings,
        assignation.Professor.id,
        Day
      )
    ...
    return res.status(500).json({
      successful: false,
      message:
        error.message || "An error occurred while updating the schedule.",
    });
  }
};
  ...
  try {
    const sched = await Schedule.findAll({
      ...
      include: [
        ...
        // 2) Direct Schedule → Room → RoomType
        {
          model: Room,
...
const deleteSchedule = async (req, res, next) => {
  try {
    ...
    if (schedule.isLocked) {
      return res
        .status(403)
        .json({ successful: false, message: "Cannot delete locked schedule." });
    }
    ...
    return res
      .status(200)
      .json({ successful: true, message: "Schedule deleted successfully." });
  } catch (error) {
    next(error);
  }
};
...
const deleteAllDepartmentSchedules = async (req, res) => {
  try {
    ...
    const assignationIds = departmentAssignations.map(
