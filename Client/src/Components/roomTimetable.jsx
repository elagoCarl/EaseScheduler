...
const RoomTimetable = () => {
    ...
    useEffect(() => {
        const fetchRooms = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:8080/room/getRoomsByDept/${deptId}`);
    ...
    // Fetch schedules when room changes
    useEffect(() => {
        if (!selectedRoom) return;
        const fetchSchedules = async () => {
            setLoadingSchedules(true);
            try {
                const response = await fetch(`http://localhost:8080/schedule/getSchedsByRoom/${selectedRoom.id}`);
    ...
    // Replace the renderMobileEvent function with this:
    const renderMobileEvent = (hour, dayIndex) => {
        if (!selectedRoom || dayIndex !== selectedDay) return null;
        const apiDayIndex = dayIndex + 1;
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center">
                ...
                <span className="ml-3 text-blue-600 font-medium">Loading rooms...</span>
            </div>
        );
    }
