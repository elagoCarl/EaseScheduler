...
const CourseManagement = () => {
  const { user } = useAuth();
  const [deptId, setDeptId] = useState(user?.DepartmentId || null);
  ...
  useEffect(() => {
    // Fetch departments only if user's department is null
  ...
  if (loading && deptId) return <div className="flex justify-center items-center h-screen">Loading courses...</div>;
    ...
    <div className="bg-slate-900 min-h-screen flex flex-col">
      ...
      <div className="flex-grow flex justify-center items-center pt-20 pb-8 px-4">
        <div className="w-full max-w-7xl my-50">
          ...
          {/* Department Selector - Only shown when user has no department */}
          {!user?.DepartmentId && departments.length > 0 && (
            <div className="bg-white p-4 rounded shadow-md mb-6">
              <label htmlFor="departmentSelector" className="block text-sm font-medium text-gray-700 mb-2">
                Select Department
              </label>
              <select
                ...
                className="w-full p-2 border border-gray-300 rounded shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200"
              >
                <option value="">-- Select a Department --</option>
                {departments.map(dept => (
                  <option key={dept.id} value={dept.id}>
                    {dept.Name} {dept.isCore ? '(Core)' : ''}
                  </option>
                ))}
              </select>
            </div>
          ...
          {!deptId ? (
            <div className="bg-white p-8 rounded shadow-md text-center">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Please Select a Department</h2>
              <p className="text-gray-600">You need to select a department to view and manage courses.</p>
            </div>
          ) : (
            <>
              {/* Search and Filter */}
      ...
      {isAddCourseModalOpen && (
        <AddCourseModal
          ...
          departmentId={deptId} // Pass selected department ID
        />
      )}
