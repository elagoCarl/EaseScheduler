...
const SectionTimetable = () => {
    ...
    // Fetch all schedules for department
    useEffect(() => {
        const fetchSchedules = async () => {
            try {
                const deptId = '1'; // Hardcoded department ID
                const response = await fetch(`http://localhost:8080/schedule/getSchedsByDept/${deptId}`);
                const { successful, data } = await response.json();
                if (successful && data.length) {
                    setSchedules(data);

                    // Extract unique programs, years, and sections from schedules
    ...
    useEffect(() => {
        ...
        const filtered = schedules.filter(schedule => {
            // Check if any ProgYrSec matches all selected filters
            return schedule.ProgYrSecs.some(prog =>
            ...
            .map(schedule => {
                ...
                return (
                    ...
                    >
                        ...
                        <div className="text-xs italic">Room {schedule.Assignation.Rooms && schedule.Assignation.Rooms[0]?.Code} - {schedule.Assignation.Rooms && schedule.Assignation.Rooms[0]?.Building}</div>
                    </div>
                );
            });
    };
