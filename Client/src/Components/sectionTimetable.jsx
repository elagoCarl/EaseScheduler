...
const SectionTimetable = () => {
    ...
    const [selectedSection, setSelectedSection] = useState('CS4A');
    ...
    // List of sections (adjust as needed)
    const sections = [
        'CS4A', 'CS5B', 'IT3A', 'CS4B', 'CS3C', 'IT4A', 'CS5A', 'CS2B', 'CS3A'
    ];
    ...
    const renderEventInCell = (hour, dayIndex) => {
        const filteredEvents = events.filter(event => {
            // Only show events for the selected section
            if (event.section !== selectedSection) return false;
        ...
        return filteredEvents.map(event => {
            const eventStartHour = parseInt(event.start.split(':')[0]);
            // Render the event only in its starting hour cell
            if (eventStartHour === hour) {
                ...
                return (
                    ...
                    >
                        ...
                        <div className="text-xs">{event.professor}</div>
                    </div>
                );
            }
            return null;
            ...
            <div className="container mx-auto px-2 sm:px-4 pt-20 pb-10 flex-1 flex items-center justify-center">
                <div className="bg-white rounded-xl shadow-lg overflow-hidden w-full">
                    ...
                    <div className="relative bg-blue-600 p-4 sm:p-6">
                        ...
                        <p className="text-blue-100 mt-1">Schedule for {selectedSection}</p>
